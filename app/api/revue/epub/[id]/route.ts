...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const issue = await prisma.revueIssue.findUnique({ where: { id, published: true } })

  if (!issue?.epubFile) {
    return NextResponse.json({ error: 'Fichier epub introuvable' }, { status: 404 })
  }

  const safeName = path.basename(issue.epubFile)
